...
**Emitted:** When the favorite chats list changes on a linked device (`favorites` app state action).
...
**Fields:**
- `timestamp` — When the change occurred on the originating device.
- `action.favorites` — The full favorite chats list, in the order the phone shows it. Each entry's `id` is a chat JID string.
- `from_full_sync` — `true` while replaying the initial app state full sync.

**Example:**
```rust
Event::FavoritesUpdate(update) => {
    let chats: Vec<&str> = update
        ...
        .collect();
    // Replace your stored list; each event carries the whole list
    println!("Favorite chats: {chats:?}");
}
```

<Note>
Each event carries the whole list, not a delta. Replace the list you held before rather than merging. An empty `favorites` list is valid and means the account has no favorite chats. To write the list from your app, call [`send_app_state_action`](/api/chat-actions#send_app_state_action) with `schemas::FAVORITES`.
</Note>
...
**Emitted:** When reaction counts, forward counts, or poll tallies change on a newsletter you're subscribed to (via `subscribe_live_updates`).
...
- `messages` — Messages whose counters changed. A message missing from an update has not changed since the previous one.
- `server_id` — Server-assigned message ID
- `reactions` — Current reaction counts (emoji code and count)
- `votes` — Current per-option poll tallies. Empty when the message is not a poll.
- `option_hash` — SHA-256 of the poll option name, the same digest as `wacore::poll::compute_option_hash`. Match it against the poll's options to find which option the tally belongs to.
- `count` — Current number of votes for that option
- `forwards_count` — How many times the message was forwarded. `None` means the server omitted the counter, which is not the same as `Some(0)`.

Live updates don't carry view or response counts. Use [`get_messages`](/api/newsletter#get_messages) to read those.
...
Event::NewsletterLiveUpdate(update) => {
    ...
    for msg in &update.messages {
        ...
        for vote in &msg.votes {
            println!("  Message {}: option {} has {} votes",
                msg.server_id, hex::encode(vote.option_hash), vote.count);
        }
        if let Some(forwards) = msg.forwards_count {
            println!("  Message {}: forwarded {} times", msg.server_id, forwards);
        }
    }
}
```
