...
pub struct HashState {
    ...
    // Set when a sync run reaches its terminal page. A bootstrap deferred
    // mid-run leaves this false even though version already moved. Also set
    // (version and hash untouched) when a sync of a collection with a real
    // version comes back empty and final, i.e. already at its head.
    ...
    // asks for patches as before, and gains the flag on its first sync.
