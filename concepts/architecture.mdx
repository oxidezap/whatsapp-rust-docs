...
- Checks dead socket on every tick that attempts a ping (not just after failures), which catches a ping that succeeded right before the connection died
- Defers the routine ping while IQ responses are pending, but once inbound data has been silent for 30s (or the watchdog expires) it sends an explicit probe ping first. A pong preserves the connection, so an ignored IQ times out without forcing a reconnect; a silent peer still triggers teardown after that one probe ([whatsapp-rust#1543](https://github.com/oxidezap/whatsapp-rust/pull/1543))
