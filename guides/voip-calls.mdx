...
<Note>
  [PR #1498](https://github.com/oxidezap/whatsapp-rust/pull/1498) added `CallHandle::resume_video(source, sink)` and `CallHandle::video_states()`, and made `start_video`'s 5-second timeout direction-aware: an unanswerable request against a peer that's already video now stands only our direction down instead of killing the call. The exact timeout is also published as `voip::VIDEO_UPGRADE_TIMEOUT`. See [Video I/O](#video-io) below.
</Note>

<Note>
  [PR #1509](https://github.com/oxidezap/whatsapp-rust/pull/1509) split the media engine from the control plane behind a neutral contract, added the `voip-control` and `voip-engine-wacore` features, and exposed [`ClientBuilder::with_voip_media_backend`](#pluggable-media-backend) so a call can run on a foreign media backend instead of the resident engine. Existing `voip-*` builds behave the same — every codec profile still pulls the resident engine in through the new composition. See [Pluggable media backend](#pluggable-media-backend) below.
</Note>
...
| Feature | Contents |
|---------|----------|
| `voip-control` | The neutral control-plane media contract, re-exported as `whatsapp_rust::voip_control`. Compiles the call flow — signaling, facade, `CallHandle`, registry — with no media engine linked. The contract is what a media backend implements; a `voip-control`-only build with no backend injected reports the typed `MediaSetupError::NoBackend` when a call tries to start media, never a panic or silent no-op. See [Pluggable media backend](#pluggable-media-backend) |
| `voip-engine-wacore` | The resident media backend that implements `voip-control`'s traits on top of `wacore`'s `CallEngine`. Composed automatically by every `voip-*` codec feature below, so an existing call profile keeps the engine it always had. Name it directly only when you want the resident engine without either bundled codec adapter |
| `voip-runtime` | Composition of `voip-control` + `voip-engine-wacore`. Owns no socket and reads no clock, so it builds on `wasm32` too — and no longer pulls in `tokio-runtime`. No audio codec, and no way onto the media wire by itself; you'll normally reach for `voip-encoded` or a codec feature below instead of naming this one directly |
...
**Either side** — re-add our video direction inside a call that's already video (peer is sending, our side is stopped):

```rust
handle.resume_video(camera_source, video_sink).await?;
```

`resume_video` re-attaches your endpoints, ungates the media plane, and announces a bare `Enabled` (`<video state=1 dec="H264" device_orientation=…>`). Unlike `start_video`, it opens no upgrade handshake and arms no timeout — the peer applies the bare `Enabled` unconditionally, which is the mute-path re-add official clients use in place of a second `11`. It refuses unless our direction is `Stopped` (or has never been enabled while the peer's is up) with no upgrade outstanding either way: a fresh upgrade belongs on `start_video`, and a pending peer request belongs on `accept_video`. The error message points at the right path (`CallError::Media("... use start_video() to upgrade")` or `"... use accept_video() instead"`). A failed send releases the plane and restores the previous direction, so a retry stays a resume.

<Note>
  [PR #1498](https://github.com/oxidezap/whatsapp-rust/pull/1498) added `resume_video`, changed the `start_video` timeout so an unanswerable upgrade no longer kills the call, and published `VIDEO_UPGRADE_TIMEOUT`. The behavior changes below only affect the timeout branch — a normal `start_video` → `UpgradeAccept` → `Enabled` flow is unchanged.
</Note>

<Note>
  An upgrade request you send with `start_video` auto-cancels after 5 seconds if the peer hasn't answered, but the announcement is now direction-aware:

  - Genuine initial upgrade (both directions were `Stopped`): the library sends `<video state=9>` (`VideoState::UpgradeCancelByTimeout`) and releases the local camera source/sink — a mutual kill, unchanged from before.
  - The peer was already video when your request went out (they ignore an `11` against their active direction, so there was never anything to answer): the library now stands only your direction down with `<video state=0>` (`VideoState::Stopped`) and releases the local source/sink while leaving the inbound sink attached, so the peer's picture keeps flowing. Their direction is untouched.

  `start_video` and `re_request_local_video` refuse up-front when the peer direction is already `Enabled` (`CallError::Media("call already has video; use resume_video() to re-add a stopped direction")`), so the direction-local timeout only fires when the peer's video comes up between the request and the timer.

  The exact 5-second value is published as `whatsapp_rust::voip::VIDEO_UPGRADE_TIMEOUT: Duration`. Arm your own answer-wait against it (a full second under is the recommended margin) instead of hardcoding the interval.
</Note>

<Tip>
  Diagnose a video negotiation from outside with `handle.video_states() -> Option<(VideoState, VideoState)>`: returns `(self, peer)` for a live call, `None` once it's gone. Use it to confirm what the state machine holds before or after driving `start_video` / `accept_video` / `resume_video` — for example, checking whether `resume_video` will refuse (`peer_state.is_upgrade_request()` means answer with `accept_video` instead).
</Tip>
...
| `handle.resume_video(source, sink).await` | Re-add our video direction inside an already-video call: re-attaches endpoints, ungates the plane, and announces a bare `Enabled`. No handshake, no timeout. Refuses unless our direction is stopped with no upgrade outstanding either way — see Video I/O above |
| `handle.video_states()` | Read back this call's `(self, peer)` `VideoState` pair, or `None` when the call is gone. Diagnostics for clients driving upgrades from outside |
...
[PR #1509](https://github.com/oxidezap/whatsapp-rust/pull/1509) separated the control plane from the media engine behind a neutral contract. The call registry stores an `Arc<dyn VoipMediaSession>`; it never names an engine type. A `VoipMediaBackend` is what a call reserves and opens a session from — the resident engine is one implementation of that trait, and you can supply another.

Reach for this in two cases:

- **Ship the call flow without the resident engine.** The `voip-control` feature compiles signaling, the facade, `CallHandle`, and the registry, and links no `wacore::voip` symbols. That is the build for a browser or embedded target that has its own media stack (a WASM engine, a native FFI codec/transport, an out-of-process daemon) and wants to route media through it instead of the resident `CallEngine`.
- **Compose with the resident engine still in.** The `voip-engine-wacore` feature adds `WacoreVoipMediaBackend` on top and is what every codec profile (`voip-mlow`, `voip-libopus`, `voip`) enables automatically. Existing builds keep the resident engine as the default backend; call `with_voip_media_backend` only to override it.

Install a backend on the builder:

```rust
use std::sync::Arc;
use whatsapp_rust::Client;
use whatsapp_rust::voip_control::{
    MediaCommand, MediaEvent, MediaSessionSpec, MediaSetupError,
    VoipMediaBackend, VoipMediaSession,
};

struct MyBackend { /* engine handle, transport, whatever the platform needs */ }

// Trait signatures use `#[async_trait]` — mirror the same cfg the trait uses so
// a wasm32 backend built around !Send handles compiles too.
#[cfg_attr(target_arch = "wasm32", whatsapp_rust::async_trait(?Send))]
#[cfg_attr(not(target_arch = "wasm32"), whatsapp_rust::async_trait)]
impl VoipMediaBackend for MyBackend {
    // Implement reserve/open/submit/stats/subscribe/close against the neutral
    // MediaCommand / MediaEvent / MediaStats types re-exported above — no
    // `wacore::voip` type appears in any signature.
    // ...
}

let client = Client::builder()
    // ...
    .with_voip_media_backend(MyBackend { /* ... */ })
    .build()
    .await?;
```

`whatsapp_rust::voip_control` re-exports every trait, command, event, spec, and port a backend implementation names — including `MediaSessionSpec`, `MediaCommand`, `MediaEvent`, `MediaStats`, `MediaOpenContext`, `MediaAudioPorts`, `MediaVideoChannels`, and `MediaSetupError` — so an external backend crate imports only from `whatsapp_rust::voip_control` and does not need `wacore` in its dependency tree.

A backend is installed at most once per client. Calling `with_voip_media_backend` twice, or building a `voip-engine-wacore` client that already has the resident backend and passing another, fails the builder with `ClientBuilderError::VoipMediaBackendAlreadyInstalled`. On a `voip-control`-only build with no backend supplied, starting media fails each call with `MediaSetupError::NoBackend` — a typed refusal, not a panic, and observable on the handle just like any other setup failure.

<Note>
  The resident backend (`WacoreVoipMediaBackend`) is the default on any `voip-engine-wacore` build and needs no explicit `with_voip_media_backend` call to be wired in. Every observable of an existing VoIP client is unchanged by the split: the engine is constructed with the same config, driven by the same `run_call`, and events reach `CallHandle::events()` on the same stream. Relay-connect failures still surface as `CallError::Connect` (now via `MediaSetupError::Connect`) and everything else as `CallError::Setup`.
</Note>
