...
Read your own reaction and poll vote on a newsletter's recent messages.
...
**Parameters:**
- `jid` — Newsletter JID
- `limit` — How many recent messages the server checks for your add-ons

**Returns:**
- `Vec<NewsletterMyAddOns>` — One entry per message you reacted to or voted on. Messages without any of your add-ons are not returned.

Poll tallies on [`NewsletterMessage::votes`](#newslettermessage) and in live updates count every follower's votes. They can't tell you whether your vote landed or what your current selection is. Use `get_my_addons` to read that back, for example after calling [`send_poll_vote`](#send_poll_vote).

A removed vote is still returned. Its `poll_vote` is `Some` with an empty `option_hashes` list. `poll_vote` is `None` only if you never voted on that message.

**Errors:** Returns `NewsletterError::InvalidRequest` without sending anything if `jid` is not a newsletter JID. A malformed server response also returns `NewsletterError::InvalidRequest`.
...
for addon in &addons {
    if let Some(reaction) = &addon.reaction {
        println!("{}: reacted {}", addon.server_id, reaction.code);
    }
    match &addon.poll_vote {
        Some(vote) if vote.option_hashes.is_empty() => {
            println!("{}: vote removed", addon.server_id);
        }
        Some(vote) => {
            println!("{}: voted for {} option(s)", addon.server_id, vote.option_hashes.len());
        }
        None => {}
    }
}
...
Subscribe to live updates for a newsletter (reaction counts, forward counts, poll tallies).
...
The server sends `Event::NewsletterLiveUpdate` events with updated reaction counts, forward counts, and poll tallies. See [NewsletterLiveUpdate](/concepts/events#newsletterliveupdate) for the event fields. You need to re-subscribe periodically when the duration expires.
...
Your own add-ons on one newsletter message, as returned by [`get_my_addons`](#get_my_addons).
...
- `server_id` — The message's server-assigned ID
- `reaction` — Your reaction, or `None` if you haven't reacted to this message
- `poll_vote` — Your poll vote, or `None` if you never voted on this message. A removed vote is `Some` with no option hashes.

### NewsletterMyReaction

Your own reaction on a newsletter message.

```rust
#[non_exhaustive]
pub struct NewsletterMyReaction {
    pub code: String,    // Reaction emoji
    pub timestamp: u64,  // When it was set (Unix seconds)
}
```

### NewsletterMyPollVote

Your own vote on a newsletter poll.
...
- `timestamp` — When the selection was last sent (Unix seconds). It matches the `t` of the server's acknowledgement for that vote.
- `option_hashes` — The selected options, each a [`compute_option_hash`](/api/polls) of an option name. These are the same keys as [`NewsletterPollVote::option_hash`](#newsletterpollvote). Empty after you remove your vote.
