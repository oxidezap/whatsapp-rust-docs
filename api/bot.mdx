...
### with_watched_ab_props

```rust
pub fn with_watched_ab_props(
    self,
    props: impl IntoIterator<Item = AbProp>,
) -> Self
```

Register extra A/B props for the [`AbPropsCache`](/api/client#ab-props-cache) to retain, on top of the flags the library reads itself. Use this to gate application behavior on a server prop with [`Client::ab_prop_enabled`](/api/client#ab_prop_enabled) — for example `AURA_PINNED_CHATS_BENEFIT_ACTIVE`, the WhatsApp Plus pinned-chats-limit gate.

The client discards every prop it does not watch while the catalog streams in, so the set is fixed on the builder before the first fetch. Calls accumulate.

<ParamField path="props" type="impl IntoIterator<Item = AbProp>" required>
  Typed flags from [`wacore::iq::abprops`](/api/wacore#a-b-props-registry). Pass a single-element array, a slice, or any iterator.
</ParamField>
...
Has no effect when [`with_ab_props_fetch(false)`](#with_ab_props_fetch) turns the whole catalog fetch off — every flag then reads as its registry default and `ab_prop_enabled` returns `None` for anything the server would have sent.
...
pub trait OriginalMessageResolver: Send + Sync {
    ...
    /// Metadata-carrying variant. Override this to return the parent
    /// message's event time so the receive path can enforce the
    /// 20-minute edit-processing window. Defaulted to the plain
    /// secret with an unknown timestamp, so an existing implementation
    /// keeps compiling and keeps the historical permissive behavior.
...
pub struct ResolvedMessageSecret {
    pub secret: [u8; 32],
    /// Parent message event time (unix seconds). `Some` enables the
    /// edit-window check; `None` leaves it unenforced.
...
<Note>
`resolve_msg_secret_with_metadata` was added in [whatsapp-rust#1504](https://github.com/oxidezap/whatsapp-rust/pull/1504). Override it (not `resolve_msg_secret`) if your store tracks the parent message's timestamp — a `Some(message_ts)` return lets the receive path apply the same `editTs < message_ts + EDIT_PROCESSING_WINDOW_SECS` check that a store row uses, so a stale edit that WhatsApp Web would drop is dropped here too. A resolver that implements only `resolve_msg_secret` keeps working and stays permissive; the app-supplied timestamp is only checked when you provide one.
</Note>
