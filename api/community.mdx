...
Link existing groups as subgroups of a community. All groups are linked with the default [`SubgroupVisibility::Visible`](#subgroupvisibility); use [`link_subgroups_with_options`](#link_subgroups_with_options) to mix visible and hidden groups in a single request.
...
**Parameters:**
- `community_jid` — JID of the parent community
- `subgroup_options` — Array of per-group link options (see [`LinkSubgroupOptions`](#linksubgroupoptions))

**Returns:**
- `LinkSubgroupsResult` — Same shape as [`link_subgroups`](#link_subgroups).
...
Create a new group that is already linked as a subgroup of a community, in one call. The subgroup is created with the default [`SubgroupVisibility::Visible`](#subgroupvisibility).
...
**Parameters:**
- `options` — Subgroup creation options (see [`CreateSubgroupOptions`](#createsubgroupoptions))
...
let participants = vec!["5511999999999@s.whatsapp.net".parse()?];
...
Options passed to [`create_subgroup_with_options`](#create_subgroup_with_options) when creating a subgroup already linked to a community.
...
**Constructors:**
```rust
// Visible-by-default options
let options = CreateSubgroupOptions::new("Announcements", &participants, community_jid.clone());

// Override the visibility
let hidden = options.with_visibility(SubgroupVisibility::Hidden);
```

### LinkSubgroupOptions

Options for linking a single existing group to a community via [`link_subgroups_with_options`](#link_subgroups_with_options).
...
**Constructor:**
```rust
let opt = LinkSubgroupOptions::new(group_jid, SubgroupVisibility::Hidden);
```
