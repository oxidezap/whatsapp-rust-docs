...
| `get_bool(prop)` | Parsed `Option<bool>`, `None` when the value is not cached (no default substitution) |
...
Only flags in the cache's *interest set* are retained when props come in. Every other server prop is discarded to avoid allocating for the ~2,000+ flags WhatsApp ships. The interest set is pre-seeded with the flags the library itself reads (see `wacore::iq::props::WATCHED`).

To gate your own code on a flag the library doesn't already watch, register it on the builder before the first `fetch_props()`. `ClientBuilder::with_watched_ab_props` (and the matching [`BotBuilder::with_watched_ab_props`](/api/bot#with_watched_ab_props)) accumulate across calls and take any `IntoIterator<Item = AbProp>`, so you can register single flags or whole slices:

```rust
use wacore::iq::abprops::web::AURA_PINNED_CHATS_BENEFIT_ACTIVE;
use whatsapp_rust::client::ClientBuilder;

let client = ClientBuilder::new(persistence_manager)
...
```rust
pub async fn ab_prop_enabled(&self, prop: AbProp) -> Option<bool>
```

Reads a watched boolean A/B prop as the server sent it. Use this to gate application behavior on the same server flags WhatsApp Web reads — for example `AURA_PINNED_CHATS_BENEFIT_ACTIVE`, the WhatsApp Plus pinned-chats-limit gate.

Returns `None` when the client holds no value for `prop`:

- The prop was not registered with [`ClientBuilder::with_watched_ab_props`](#watching-additional-flags), so the cache dropped it while the catalog streamed in.
- The catalog has not been fetched yet on this connection. `fetch_props()` runs automatically on connect unless [`with_ab_props_fetch(false)`](/api/bot#with_ab_props_fetch) turned it off.
- The server did not include `prop` in its response.

Unlike [`AbPropsCache::is_enabled`](#ab-props-cache), `ab_prop_enabled` does not substitute the registry default, so a caller can tell "off" from "unknown". A cached value is truthy when it equals `"1"`, `"true"`, or `"enabled"` (case-insensitive).

<ParamField path="prop" type="AbProp" required>
  Typed flag from [`wacore::iq::abprops`](/api/wacore#a-b-props-registry).
</ParamField>
...
match client.ab_prop_enabled(AURA_PINNED_CHATS_BENEFIT_ACTIVE).await {
...
<Note>
The AB props cache itself is internal — the library automatically checks relevant flags when performing group operations like `create_group` and `add_participants`. You only need `with_watched_ab_props` + `ab_prop_enabled` when your application needs to read a server gate the library doesn't already watch.
</Note>
...
<ParamField path="query" type="InfoQuery" required>
  IQ query containing stanza type, namespace, content, and optional timeout. The timeout (75s when `None`) covers writing the stanza to the socket and waiting for the response together, so a write that never completes still returns `IqError::Timeout`.
</ParamField>
