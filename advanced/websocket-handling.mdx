...
async fn sender_task(
    ...
    loop {
        ...
        // ... encrypt_frame_into loop ...

        // Before the write starts: stats.arm_dead_socket_deadline(), so a
        // write that never completes still arms the watchdog. Then a single
        // transport.send(out_buf).
...
<Note>
**A socket that stops draining no longer hangs the connection ([whatsapp-rust#1547](https://github.com/oxidezap/whatsapp-rust/pull/1547)).** A socket can stay established but stop accepting writes, for example after a laptop resumes from suspend or the network changes underneath it. The socket buffers a few frames, then blocks the next write forever without an error. Previously, an IQ's deadline started only after its write returned, so the keepalive ping waited on that write forever and the connection stayed "up" until the process restarted. Now the IQ deadline covers the write and the wait together, and the watchdog arms before the write starts. A wedged ping times out after 20s, and the watchdog reconnects on the next tick.
</Note>
...
The watchdog is anchored to `SessionStats::first_send_since_recv` — the **first** send since the last receive, not the most recent send. `SessionStats::arm_dead_socket_deadline()` only stores a new anchor when the current one is unset or stale (`<=` the last-received timestamp); once armed, further sends leave it in place. The noise sender calls it when a batch enters the transport, *before* the write completes, matching WA Web, which arms the deadline at the send call. `record_frame_sent` calls it again after a successful write, which is a no-op once the anchor is set. Every receive clears the anchor (`None`), and the next send re-arms it. Anchoring on the most recent send instead (the pre-fix behavior) let continued outgoing traffic — messages, receipts, presence — keep pushing the deadline forward, hiding a half-open socket (a peer that silently disappeared while writes still buffer and reads hang) for as long as the app kept emitting frames.
